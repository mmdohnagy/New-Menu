...
import { Calendar, Clock, AlertCircle, CheckCircle2, Loader2, ChevronDown, Globe, Shield, Settings, MessageSquare, Database } from 'lucide-react';
...
export default function TechnicalBackOfficeView() {
  ...
  const [activeTab, setActiveTab] = useState<'busy' | 'database' | 'technical' | 'requests'>(user?.role_name === 'Call Center' ? 'database' : 'busy');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.isAuthError) return;
      showToast(lang === 'en' ? 'Failed to create record' : 'فشل في إنشاء السجل', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const renderTabSwitcher = () => (
    <div className="flex items-center gap-4 bg-white dark:bg-zinc-900 p-2 rounded-2xl border border-zinc-100 dark:border-zinc-800 w-full sm:w-fit self-center overflow-x-auto no-scrollbar">
      <div className="flex items-center gap-4 min-w-max">
        ...
        {(user?.role_name === 'Technical Back Office' || user?.role_name === 'Manager' || user?.role_name === 'Super Visor' || user?.role_name === 'Restaurants') && (
          <button
  ...
  return (
    <div className="h-[calc(100vh-120px)] flex flex-col gap-6 overflow-y-auto pr-2">
      ...
      {activeTab === 'technical' ? (
        ...
        <PendingRequestsView filterType="busy_branch" />
      ) : (
        <motion.div 
